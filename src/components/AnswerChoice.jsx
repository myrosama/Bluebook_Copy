...
export default function AnswerChoice({
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        onClick={onSelect}
        className={`answer-card flex-1 flex items-center gap-3 px-4 py-[10px] rounded-lg border bg-white text-left ${
          selected ? 'border-bb-blue ring-2 ring-bb-blue' : 'border-[#aaaaaa]'
        } ${struck ? 'opacity-60' : ''}`}
      >
        <span
          className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[11px] font-semibold border ${
            selected
              ...
              : 'bg-white text-bb-textDark border-[#aaaaaa]'
          }`}
        >
          {letter}
        </span>
      ...
      <button
        ...
        className="w-5 h-5 rounded-full border border-[#aaaaaa] text-[11px] font-semibold inline-flex items-center justify-center text-bb-textDark hover:bg-gray-100 relative shrink-0"
      >
        {letter}
