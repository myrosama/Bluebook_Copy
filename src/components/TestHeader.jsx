    ...
    <header className="relative h-[64px] bg-bb-headerBg flex items-stretch px-8">
      ...
      <div className="flex flex-col justify-center min-w-[320px]">
        ...
        <button
          ...
          className="text-[13px] font-semibold text-bb-textDark mt-1 inline-flex items-center gap-1 w-fit hover:underline"
        >
          Directions
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" className={showDirections ? 'rotate-180' : ''}>
            <path d="m6 9 6 6 6-6" />
          </svg>
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        {!hidden ? (
          <div className="text-[20px] font-semibold tabular-nums text-bb-textDark leading-none">
            {formatTime(timeLeft)}
          </div>
        ) : (
        ...
        <button
          ...
          className="mt-[10px] text-[11px] font-semibold px-3 py-[1px] rounded-full border border-bb-textDark text-bb-textDark hover:bg-white"
        >
          {hidden ? 'Show' : 'Hide'}
        </button>
      </div>
